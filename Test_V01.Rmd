...
```{r}
data$TO <- ifelse(data$PERIOD > 4, 1, 0)
```
...
###Streak
Aquesta variable indica si el jugador es troba en ratxa, considerem aquests moments quan en un mateix quart ha anotat més de 10 punts.
...
```{r}
df <-data %>% group_by(GAME_ID, player_id, FGM)
...
df_tirs <- inner_join(df %>% filter(FGM ==1) %>% summarise(TR = n(),
                      ...
                      by = c("GAME_ID", "player_id"), copy = TRUE)
#Tirs Intentats
...
save(df_tirs, file = "data_tirs.csv")
```

```{r}
rows <- c("LOCATION","W","FINAL_MARGIN","PERIOD","TOUCH_TIME",
          "SHOT_DIST","SHOT_DIST_C","PTS_TYPE","SHOT_RESULT",
          "CLOSE_DEF_DIST","CLOSE_DEF_DIST_C","CLUTCH")
df_nba <- data[, rows]
save(df_nba, file = "data_nba.csv")
```
